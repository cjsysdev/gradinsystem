...
of paper/photo-upload. Plan is to reuse `assessments.given` (currently
unused) for instructor widget config, `classworks.code` (already
text-capable) for student submissions, and add one new column
`assessments.widget_type` to select which interface renders. Build order and
full JSON schemas for each widget are in the plan doc — do not re-derive
them from scratch, follow what's specified there unless told otherwise.
